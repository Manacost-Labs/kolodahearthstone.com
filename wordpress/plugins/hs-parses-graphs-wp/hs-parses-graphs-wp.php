...
define( 'HS_PARSES_GRAPHS_VERSION', '1.1.14' );
...
function hs_parses_graphs_is_allowed_dataset_host( $url ) {
	$host = wp_parse_url( $url, PHP_URL_HOST );

	return in_array( $host, array( 'api.kolodahearthstone.com' ), true );
}

function hs_parses_graphs_get_dataset_url( $source, $api_url ) {
	$api_url = hs_parses_graphs_sanitize_api_url( $api_url );

	if ( ! hs_parses_graphs_is_allowed_dataset_host( $api_url ) ) {
	...
	return add_query_arg(
		...
		rest_url( 'hs-parses-graphs/v1/datasets/' . sanitize_key( $source ) )
	);
}
...
function hs_parses_graphs_enqueue_hs_tooltip_assets() {
	if ( ! function_exists( 'hs_smart_tooltip_enqueue_assets' ) ) {
		return;
	}

	try {
		$tooltip_file = ( new ReflectionFunction( 'hs_smart_tooltip_enqueue_assets' ) )->getFileName();
	} catch ( ReflectionException $e ) {
		$tooltip_file = '';
	}

	if ( ! $tooltip_file || ! file_exists( $tooltip_file ) ) {
		return;
	}

	$base_url = trailingslashit( plugin_dir_url( $tooltip_file ) ) . 'assets/';
	$version  = defined( 'HS_SMART_TOOLTIP_VERSION' ) ? HS_SMART_TOOLTIP_VERSION : HS_PARSES_GRAPHS_VERSION;
...
function hs_parses_render_vs_radar( $atts ) {
	...
	$api_url    = hs_parses_graphs_sanitize_api_url( $a['api_url'] );
	$radars_url = hs_parses_graphs_get_dataset_url( 'vicious_syndicate_radars', $api_url );
	$archetypes_url = hs_parses_graphs_get_archetype_translations_url( $a['archetypes_api_url'] );
	...
	hs_parses_graphs_enqueue_hs_tooltip_assets();

	ob_start();
	?>
	<div class="hs-vs-radar-wrapper" 
		data-api-url="<?php echo esc_url( $api_url ); ?>"
		data-radars-url="<?php echo esc_url( $radars_url ); ?>"
		data-archetypes-url="<?php echo esc_url( $archetypes_url ); ?>"
...
function hs_parses_graphs_register_rest_routes() {
	...
	register_rest_route(
		'hs-parses-graphs/v1',
		'/datasets/(?P<source>[a-z0-9_\-]+)',
		array(
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => 'hs_parses_graphs_rest_dataset',
			'permission_callback' => '__return_true',
			'args'                => array(
				'source' => array(
					'type'              => 'string',
...
function hs_parses_graphs_rest_dataset( WP_REST_Request $request ) {
	$source = sanitize_key( (string) $request->get_param( 'source' ) );

	if ( 'vicious_syndicate_radars' !== $source ) {
		return new WP_Error(
			'hs_parses_graphs_dataset_not_allowed',
	...
	$base = rawurldecode( (string) $request->get_param( 'base' ) );
	$base = hs_parses_graphs_sanitize_api_url( $base );

	if ( ! hs_parses_graphs_is_allowed_dataset_host( $base ) ) {
		$base = hs_parses_graphs_get_default_api_url();
	}

	$cache_key = 'hs_parses_dataset_' . md5( $base . '|' . $source );
	$cached    = get_transient( $cache_key );
	if ( is_array( $cached ) ) {
		return rest_ensure_response( $cached );
	}

	$data = hs_parses_graphs_fetch_json( $base . '/datasets/' . rawurlencode( $source ), 30 );
	if ( is_wp_error( $data ) ) {
		return $data;
	}
