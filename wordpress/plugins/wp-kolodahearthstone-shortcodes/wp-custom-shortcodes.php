<?php
/*
Plugin Name: KolodaHearthstone: Custom Shortcodes
Description: Плагин для добавления кнопки "Кастомные шорткоды" над редактором
Version: 1.0
Author: Manacost
*/
...
class WP_Custom_Shortcodes {
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        wp_enqueue_script(
            ...
            plugins_url('custom-shortcodes.js', __FILE__),
            ['jquery'],
            filemtime(plugin_dir_path(__FILE__) . 'custom-shortcodes.js'),
            true
        );
        
        wp_enqueue_style(
            'mtp-custom-shortcodes',
            plugins_url('custom-shortcodes.css', __FILE__),
            [],
            filemtime(plugin_dir_path(__FILE__) . 'custom-shortcodes.css')
        );
        
        wp_enqueue_style(
            'mtp-tinymce-modals',
            plugins_url('tinymce-modals.css', __FILE__),
            [],
            filemtime(plugin_dir_path(__FILE__) . 'tinymce-modals.css')
        );
    }
