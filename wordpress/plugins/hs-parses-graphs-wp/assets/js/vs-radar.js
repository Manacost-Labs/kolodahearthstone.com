(function() {
  ...
  function initRadarChart(wrapper, wrapperIndex) {
    ...
    var radarsUrl = wrapper.getAttribute('data-radars-url') || (apiUrl + '/datasets/vicious_syndicate_radars');
  ...
  function loadRadarData(url) {
    ...
    if (!radarPromises.has(key)) {
      radarPromises.set(key, fetch(key, { credentials: 'omit' }).then(function(res) {
        if (!res.ok) throw new Error('HTTP ' + res.status);
        return res.json();
      }).catch(function(err) {
        radarPromises.delete(key);
        throw err;
      }));
    }

    return radarPromises.get(key);
  }
